...
DISTANCE_URBAINE = (
    "dist_gcu_group_km_T15"  # préparée par l'étape 00 (d_geographie/urbain.py)
)
...
def matrice_conception(
    communes: pd.DataFrame, colonnes: list[str] | None = None
) -> pd.DataFrame:
    ...
    X = (
        ...
        .copy()
        .join(pd.get_dummies(communes[NIVEAU_DENSITE], prefix="DENS7"))
    )
    if colonnes is None:
        return X
...
def jeu_entrainement(
    communes: pd.DataFrame, capteurs: pd.DataFrame, labels: pd.Series
) -> JeuEntrainement:
    ...
    lignes["poids"] = 1.0 / lignes.groupby("id_commune_str")["id_site"].transform(
        "size"
    )
    jointure = lignes.merge(
        communes, left_on="id_commune_str", right_on="code_commune", how="inner"
    ).reset_index(drop=True)
    X = matrice_conception(jointure)
    valide = X.notna().all(axis=1)
    return JeuEntrainement(
        X[valide],
        jointure.loc[valide, "cluster"].astype(int),
        jointure.loc[valide, "poids"],
        jointure.loc[valide, "id_commune_str"],
    )
...
def effectifs_par_cluster(jeu: JeuEntrainement) -> pd.DataFrame:
    ...
    return (
        pd.DataFrame(
            {
                "lignes": jeu.y.value_counts().sort_index(),
                "communes": jeu.groupes.groupby(jeu.y).nunique(),
            }
        )
        .fillna(0)
        .astype(int)
